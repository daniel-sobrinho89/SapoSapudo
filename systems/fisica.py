...
class SistemaFisica:
    def aplicar_gravidade(self, entidade, dt):
        gravidade = entidade.gravidade * (1 - (entidade.sustentacao_restante * 0.80))

        entidade.vel_y += gravidade * dt

    def aplicar_gravidade_simples(self, entidade, dt, intensidade=4.0):
        entidade.vel_y += intensidade * dt

    def integrar_movimento(self, entidade, dt):
        entidade.x += entidade.vel_x * dt

        entidade.y += entidade.vel_y * dt

    def resolver_limites(self, entidade, clima_service):
        altura_maxima = entidade.limite_chao - (clima_service.wind_speed * 8)

        altura_maxima = max(120, altura_maxima)
        ...
        if entidade.y >= entidade.limite_chao:
            pousou_agora = not entidade.no_chao
    ...
    def aplicar_forca_vento(
        ...
        if hasattr(entidade, "vel_x"):
            ...
            entidade.vel_x = prev + delta_v * dt
