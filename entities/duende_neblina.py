...
class DuendeNeblina:
    ...
    def mover_arraste(self, mouse_x, mouse_y):
        ...
        self.x, self.y = mover_com_offset(
            mouse_x, mouse_y, self.offset_drag_x, self.offset_drag_y
        )
    ...
    def iniciar_teleporte(self, destino_x, destino_y):
    ...
    def atualizar_teleporte(self, dt):
        ...
        progresso = self.teleporte_tempo / self.teleporte_duracao
        ...
        if self.teleporte_fase == "sumindo":
            ...
            if progresso >= 1:
                if self.teleporte_violao_alvo and self.teleporte_violao_alvo.caindo:
                    violao = self.teleporte_violao_alvo

                    queda_prevista = violao.velocidade_queda * self.teleporte_duracao
                    ...
                    self.x = self.teleporte_destino_x

                    self.y = self.teleporte_destino_y
    ...
    def consegue_alcancar_antes_da_queda(self, violao):
        distancia = math.hypot(violao.x - self.x, violao.y - self.y)

        tempo_voo = distancia / self.velocidade_resgate
    ...
    def teleportar_para_violao(self, violao):
        ...
        self.iniciar_teleporte(violao.x, violao.y)
    ...
    def atualizar_resgate(self, dt):
        ...
        if not self.violao_em_maos:
            ...
            self.x += (dx / max(1, distancia)) * self.velocidade_resgate * dt

            self.y += (dy / max(1, distancia)) * self.velocidade_resgate * dt
        ...
        violao.x = self.x + self.offset_violao_x

        violao.y = self.y + self.offset_violao_y
    ...
    def verificar_violao_apos_acordar(self, sapo):
        ...
        if fora_do_lugar:
            self.iniciar_resgate_violao(violao)
    ...
    def _atualizar_fator_sono_visual(self, dt, sapo):
        if (
            not self.animacoes.indo_para_frasco
            and not self.animacoes.descendo_para_dormir
            and not self.animacoes.dormindo
            and not self.esta_dentro_do_frasco(self.area_frasco_atual)
        ):
    ...
    def _atualizar_estado_dormindo(self, dt, clima_service, frasco_rect):
        self.y = self.y_sono
    ...
    def _atualizar_entrada_frasco(self, dt):
        ...
        dx = self.x_entrada_frasco - self.x

        dy = self.y_entrada_frasco - self.y
        ...
        if distancia > 5:
            self.x += (dx / distancia) * velocidade_aproximacao * dt

            self.y += (dy / distancia) * velocidade_aproximacao * dt
    ...
    def _atualizar_descida_sono(self, dt):
        ...
        self.y += self.velocidade_descida * dt

        altura_restante = max(0, self.y_sono - self.y)

        fator_voo = max(0, min(1, altura_restante / 80))

        self.y += math.sin(self.tempo * 8) * fator_voo
    ...
    def atualizar(self, dt, sapo, pote_x, pote_y, clima_service, frasco_rect, ambiente):
        ...
        if (
            getattr(self, "animacoes", None) is not None
            and self.animacoes.dormindo
            and (
        ...
        if self.animacoes.dormindo:
            self._atualizar_estado_dormindo(dt, clima_service, frasco_rect)
